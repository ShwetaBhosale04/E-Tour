...
const Login = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:8080/crud/signup")
            .then(res => res.json())
    ...
    const onLogin = () => {
        ...
        user.map(val => {
            if (val.email === id.value && val.password === pass.value) {
                ...
                // window.sessionStorage.setItem("mydata", JSON.stringify(obj));
                // window.sessionStorage.getItem("mydata");

                fetch("http://localhost:8080/crud/search/" + obj.password)
                    .then(res => res.json())
                    .then((result) => {
                        setUserenter(result);
                    });

                // window.sessionStorage.setItem('userdata', JSON.stringify(userenter));
